...
fn main() {
    ...
    while !STOP.load(Ordering::Relaxed) {
        ...
        let cap_shm = shm;
        let play_shm = shm;
        // Exit the stream to re-lock the device on shutdown or a CoreAudio rate
        // change (the plugin publishes its chosen rate into shm.sample_rate).
        ...
        let res = dev.run(
            ...
            move |planar: &[Vec<i32>], frames: usize| unsafe {
                ...
                if STOP.load(Ordering::Relaxed) || ((want == 44100 || want == 48000) && want != r) {
        ...
        if let Err(e) = res { eprintln!("sl3d: audio ended: {e}"); }

        unsafe { (*shm).device_present.store(0, Ordering::Relaxed); }
        println!("sl3d: device closed");

        // Backoff before reopening so a transient failure can't runaway-churn.
        if !STOP.load(Ordering::Relaxed) { std::thread::sleep(Duration::from_millis(500)); }
    }
